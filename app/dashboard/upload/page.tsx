            ...
            getFilteredDocuments().map((doc) => (
              <div key={doc.id} className="text-gray-600 text-sm flex flex-col gap-2">
                <p className="flex justify-start items-center gap-2">
                  <span
                    className={`px-3 py-1 rounded-full text-sm font-medium shadow-md transition-all
                      ${
...
export default function UploadDocuments() {
  ...
  useEffect(() => {
    if (docTypes?.items?.length) {
      setDocTypeIds({
        passport: docTypes.items.find((type) => type.name.toLowerCase() === "passport")?.id || null,
        entity1: docTypes.items.find((type) => type.name.toLowerCase() === "articles of organization")?.id || null,
        entity2: docTypes.items.find((type) => type.name.toLowerCase() === "operating agreement")?.id || null,
        entity3: docTypes.items.find((type) => type.name.toLowerCase() === "proof of ein")?.id || null,
        bank: docTypes.items.find((type) => type.name.toLowerCase() === "bank statements")?.id || null,
      });
    }
  }, [docTypes]);
  ...
  return (
    <div className="min-h-screen bg-gray-100 p-6">
      <div className="max-w-3xl mx-auto bg-white shadow-lg rounded-xl p-6">
        <h1 className="text-2xl font-semibold text-gray-800 mb-4">Upload Documents</h1>
